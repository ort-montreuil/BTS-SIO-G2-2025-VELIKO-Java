    ...
    public class VelikoController implements Initializable {
        ...
        public void initialize(URL url, ResourceBundle resourceBundle) {
            ...
            tcAdminName.setCellValueFactory(new PropertyValueFactory<>("nom"));

            try {
                tvAdmin.setItems(FXCollections.observableArrayList(userController.getUsersWithAdValidationFalse()));
            } catch (SQLException e) {
                throw new RuntimeException(e);
            }
        ...
        @FXML
        public void btnValidationAdminClicked(Event event) throws SQLException {
            User selectedAdmin = (User) tvAdmin.getSelectionModel().getSelectedItem();

            if (selectedAdmin != null) {

                userController.editBooleanAdValidationTrue(selectedAdmin.getEmail());
                ...
                alert.setHeaderText(null);
                alert.setContentText("Veuillez sélectionner un utilisateur dans le tableau.");
                alert.showAndWait();
            }
        }
